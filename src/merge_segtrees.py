...
class CNA_Merge:
    ...
    def construct_clonal_tree():
        pass 

    def fit(self, data, top_n=3):
        all_trees = self.enumerate_trees()
        all_results = []
        for tree in all_trees:
            cost, ct, ca = self.construct_clonal_tree(tree)
            all_results.apppend((cost,ct,ca))
    ...
    def get_desc_dict(self):
        ...
        cna_nodes_T1 = set([n for n in self.T1 if n > self.k])
        cna_nodes_T2 = set([n for n in self.T2 if n > self.k]) 
        desc_nodes = {}  
        ...
        for u in self.T_m:
            ...
            for d in desc_sets:
                if len(d) > 0:
                    cna_nodes1 = self.get_desc(u, d, self.T1, self.k)
                    cna_nodes1 = set(cna_nodes1).intersection(cna_nodes_T1)
                    for n in cna_nodes1:
    
                        cna_nodes_T1.remove(n)
                    cna_nodes2 = self.get_desc(u, d, self.T2,self.k)  
        ...
        for u in self.T_m:
            ...
            cna_nodes2 = set()
            for v in T2.successors(u):
                if v not in T_m and v in cna_nodes_T2:
                    cna_nodes2.add(v)
